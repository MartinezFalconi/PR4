    ...
    ajax.onreadystatechange = function() {
        ...
        if (ajax.readyState == 4 && ajax.status == 200) {
            ...
            for (let i = 0; i < respuesta.length; i++) {
                ...
                var borrar = "/borrar/"
                var url_global = "{{url(" + borrar + respuesta[i].id_restaurant + ")}}";
                tabla += '<div class="col">';
                ...
                tabla += '<td>';
                tabla += '<form method="get" action="actualizar/' + respuesta[i].id_restaurant + '">';
                tabla += '<button type="submit" class="boton-actualizar" onclick="return">Actualizar</button>';
                tabla += '</form>';
                tabla += '</td>';
                tabla += '<button class="boton-borrar" onclick="deleteRestaurant(' + respuesta[i].id_restaurant + ')">Borrar</button>';
                tabla += '</td>';
                tabla += '</div>';
                tabla += '</div>';
                tabla += '</div>';
...
function deleteRestaurant(id_rest) {

    var mensaje = document.getElementById('mensaje');
    var ajax = new objetoAjax();
    var token = document.getElementById('token').getAttribute('content');
    ajax.open('POST', 'deleteRestaurant', true);
    var datos = new FormData();
    datos.append('id_restaurant', id_rest);
    ...
    ajax.onreadystatechange = function() {
        if (ajax.readyState == 4 && ajax.status == 200) {
            ...
            if (respuesta.resultado == 'OK') {
                mensaje.innerHTML = 'Restaurante Eliminado';
            } else {
